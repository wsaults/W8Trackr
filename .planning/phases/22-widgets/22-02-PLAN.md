...
must_haves:
  ...
  key_links:
    - from: "W8Trackr/Views/WeightEntryView.swift"
      to: "WidgetCenter"
      via: "reloadTimelines call after add/edit/delete"
...
2. Add widget refresh calls to `WeightEntryView.swift`:
   - WeightEntryView handles both add and edit modes via an optional `existingEntry` parameter
   - After successful save (both add and edit), add `import WidgetKit` at top
   - Call `WidgetCenter.shared.reloadTimelines(ofKind: "WeightWidget")` after save
   - Also call reloadTimelines after delete (if delete functionality exists in this view)

3. Add widget refresh calls to `SettingsView.swift`:
...
- WeightEntryView is a unified view that handles both adding new entries and editing existing ones
  </action>
  <verify>
...
3. Grep for reloadTimelines calls: should appear in WeightEntryView.swift and SettingsView.swift
  </verify>
  <done>
